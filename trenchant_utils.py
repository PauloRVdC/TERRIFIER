...
def get_metric(metric, true, pred, average='macro', labels=None):
    if metric == 'acc':
        return accuracy_score(list(true), list(pred))
    elif metric == 'precision':
        return precision_score(list(true), list(pred), average=average, labels=labels)
    elif metric == 'recall':
        return recall_score(list(true), list(pred), average=average, labels=labels)
    elif metric == 'f1':
        return f1_score(list(true), list(pred), average=average, labels=labels)
...
def gcn(G, interval, i, dimensions=512, label_feature='trend', type_feature='node_type', event_feature='event', label_number_feature='type_code', embedding_feature='f'):
    node_list = []
    for node in G.nodes():
    ...
    l_sizes = [1024, 1024, dimensions, nb_classes]
...
def make_hin(df, 
             embedding_feature='embedding', id_feature='EventId', date_feature='WeekYear', date_value_feature='Date', 
    ...
    for index,row in tqdm(df.iterrows()):
        ...
        # embedding
        G.nodes[node_id]['embedding'] = row[embedding_feature]
    return G
...
def get_lstm(dimX, dimY):
    model = Sequential()
    model.add(Dense(dimX, activation='relu', input_shape=(1, dimX)))
    model.add(CuDNNLSTM(round(dimX/8)))
